(() => {
  // Theme toggle
  const savedTheme = localStorage.getItem('lo-theme');
  if (savedTheme) {
  ...
  const savedRtl = localStorage.getItem('lo-rtl');
  if (savedRtl === 'rtl') {
  ...
  function updateRtlBtns() {
    ...
    document.querySelectorAll('[data-rtl]').forEach(btn => {
      ...
      btn.setAttribute('aria-label', isRtl ? 'Switch layout direction' : 'Switch layout direction');
  ...
  document.querySelectorAll('[data-rtl]').forEach(btn => {
    btn.onclick = () => {
      ...
      localStorage.setItem('lo-rtl', newDir);
      updateRtlBtns();
    ...
    form.onsubmit = e => {
      ...
      if (chatList) {
        ...
        setTimeout(() => {
          ...
          reply.innerHTML = `Thank you for your message! Maria will review this and reply shortly.<span class="chat-time">${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>`;
  ...
  // Back to Top Button (Appears after scrolling ~10 lines / 80px)
  ...
  // Floating WhatsApp Quick CTA Widget (Inspired by Pravesh Montessori)
  if (!document.querySelector('.floating-whatsapp') && !document.body.classList.contains('portal')) {
    const waBtn = document.createElement('a');
    waBtn.className = 'floating-whatsapp';
    waBtn.href = 'https://wa.me/?text=Hello%20Pravesh%20Montessori%20Learning%20Pod';
    waBtn.target = '_blank';
    waBtn.rel = 'noopener';
    waBtn.setAttribute('aria-label', 'Chat on WhatsApp');
    waBtn.innerHTML = '💬 Chat with Us';
